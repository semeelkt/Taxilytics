# Verify finregindia.vercel.app in Google Search Console & Submit sitemap
...
4. Enter your site URL exactly: `https://finregindia.vercel.app/` and click "Continue".
5. Verification options:
   - Use the HTML file upload method: download the verification file and upload it to the root (`https://finregindia.vercel.app/`). For a static Vercel site you can add the file to the repository root so it is deployed.
...
1. In Search Console, open your verified property `https://finregindia.vercel.app/`.
...
Notes & tips:
- Ensure `robots.txt` references the sitemap (already added in this repo: `robots.txt` contains `Sitemap: https://finregindia.vercel.app/sitemap.xml`).
...
- After verification and sitemap submission, monitor "Coverage" and "Performance" in Search Console to track impressions and queries for the keyword "Finreg".
